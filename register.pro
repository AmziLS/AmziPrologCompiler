...
unl$rid(File, UserS, OrgS, InfoS, ProdS, KeyS) :-
 ...
 note(File),
  ...
  (file_exists(File, 1) ->   % does it exist and is it a .xpl file
     true
     ;
     amzi_system:err$exec(execE, `Unlock can't find system file, run from Amzi! abin directory.  `,
              File, unlock(File))
  ),
...
unl$fst(H, Char, NumTimes, Offset) :-
  ...
  (FileChar = Char ->
    ...
    SLen = NumTimes1
  ;
    ('{sys}loff'(_) -> retractall('{sys}loff'(_)); true),
    cntr_set(1, 1),
    fail
  ),
...
reverse([], R, R).
reverse([A|Z], X, R) :- reverse(Z, [A|X], R).

append([], Z, Z).
append([A|X], Y, [A|Z]) :- append(X, Y, Z).

:- end_body(amzi_register).
